<?php 
	$size_quantity = 0;
	$iparxonta_query = "SELECT * FROM `a_materials_packages`, `a_packages` WHERE `material_id`= ".$row['id']." AND `size`= ".$size_row['id']." AND `a_packages`.`subunit_id` = ".$_GET['subunit']." AND `a_packages`.`id` = `a_materials_packages`.`package_id`";
