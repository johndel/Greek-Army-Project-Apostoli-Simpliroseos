...
<?php $sql = 'SELECT * FROM b_packages ORDER BY number ASC'; ?>
<?php $result = $conn->query($sql) or die(mysqli_error()); ?>

<h4>Επεξεργασία - Προσθήκη - Διαγραφή Υλικών Πίνακα Β</h4>
<h3>Σύνολο: <?php echo $result->num_rows; ?> δέματα</h3>
<a href="new.php" class="btn btn-success"><i class="icon-plus-sign"></i> Προσθήκη νέου δέματος - Πίνακα B</a>
...
while($sub = $subunits_result->fetch_assoc()) {
    if($sub['id'] == $row['subunit_id']) { echo smallsubunit($sub['subunits']); }
  } ?>
</td>
