...
<?php
	...
	$package = select_atable_package_id($_GET['package_id']);
...
Στην συντήρηση και συσκευασία των κατωτέρω υλικών:
	<?php $test = select_all_records("a_materials_packages", "package_id", $_GET['package_id']); ?>
	<br />
...
<center>
	...
	<tr>
		<td>&nbsp;</td><td>ΥΛΙΚΑ ΠΙΝΑΚΑ "Α"</td><td></td><td></td>
	</tr>
	<?php 
	foreach($test as $value => $row) { ?>
		<tr>
			...
			<?php $mat_name = select_record("a_materials", $row['material_id']); ?>
			<?php 
			if($mat_name[0] != 30 && $mat_name[0] != 31) {
				echo "ΠΛΗΝ ";
			}
			?>
			
			<?php echo $mat_name[1]; ?></td><td style="text-align: center"><?php if($mat_name[0] == 30 || $mat_name[0] == 31) { $size = select_record("a_sizes", $row['size']); echo $size[2]; /* $size = select_atable_material_size($row['size'], $_GET['package_id']); echo $size[0]; */ } else { echo "-"; }?></td>
			<td style="text-align: center"><?php echo get_quantity_type("a", $mat_name[0]); ?>: <?php echo $row['quantity']; ?></td>
		</tr>
