<?php
	...
	$test = select_all_records_a_order("a_materials_packages", "package_id", $_GET['package_id']);	
	foreach($test as $value => $row) {
		$tmp_counter = $tmp_counter + 1;
		$top_td = $top_td + 34;
		
		imagettftext($im, 14, 0, $left_td+6, $top_td+2, $black, "arial.ttf", $tmp_counter);
			$mat_name = select_record("a_materials", $row['material_id']); 
			if(($mat_name[0] != 30 && $mat_name[0] != 31)) {
				imagettftext($im, 10, 0, $left_td + 62, $top_td, $black, "arial.ttf", "ΠΛΗΝ ");
			} 
			$size_number = select_record("a_sizes", $row['size']); 
			
			imagettftext($im, 10, 0, $left_td + 102, $top_td, $black, "arial.ttf", $mat_name[1]);
			
			$quantitytype = get_quantity_type("a", $mat_name[0]);
			imagettftext($im, 12, 0, $left_td + 360, $top_td, $black, "arial.ttf", $quantitytype.": ".$row['quantity']);

			if($mat_name[0] == 30 || $mat_name[0] == 31) { 
				//$size = select_atable_material_size($mat_name[0], $_GET['package_id']);  
				//$tmp_size_print = $size[0]; 
				$size_number = select_record("a_sizes", $row['size']);
				$tmp_size_print = $size_number[2];			
			} 
			else { $tmp_size_print = "-"; }
