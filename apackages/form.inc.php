...
<form id="a_packages" method="post" action="">
<div id="notice"></div>
<p>
...
<a href="javascript:;" class="add-material-a btn btn-success"><i class="icon-plus"></i> Υλικά:</a> <span>(Τα υλικά αφαιρούνται από το δέμα πλην από τις φανέλλες και τα σλιπ)</span>
<br />
...
<?php if($edit_mode) { ?>
<?php  
	$select_materials = 'SELECT * FROM a_materials_packages WHERE package_id = ? ORDER BY material_id, size, quantity';
    if ($stmt->prepare($select_materials)) {
	  $stmt->bind_param('i', $_GET['package_id']);
	  $results = $stmt->execute();
	  $stmt->store_result();
	  $stmt->bind_result($assoc_id, $mate_id, $pack_id, $size, $quantity);
...
<br />
<p>
  <input type="submit" class="btn btn-primary" name="submit_a_package" value="<?php echo $submit_text; ?>" id="submit_package">
</p>
</form>
<br />
<a class="btn" href="index.php"><i class="icon-arrow-left"></i> Πίσω</a>