	...
	<?php
		...
		$tmp_prov = $prov_material + $apomiwsi*$imatismou_row['quantity'];
	...
	<tr class="<?php $tmp_counter = $tmp_counter + 1; if($tmp_counter % 2 == 0) { ?>even<?php } else { ?>odd<?php } ?>">
		...
		<td><?php echo $tmp_prov_total_sum = $prov_sum_material + $prov_material + $apomiwsi*$imatismou_row['quantity']; ?></td>
		<td <?php materials_check($tmp_prov_total_sum, $tmp_total_sum); ?>><?php echo $tmp_sum; // count_sum_b_materials($imatismou_row['id']); ?></td>
		<td>&nbsp;</td>	
	</tr>
	<?php } ?>
