<?php 
  include("../partials/database-connect.inc.php");
	...
	$sql = 'SELECT id, name, quantity, efedroi FROM a_materials WHERE id=?';
...
<?php include("../partials/menu.inc.php") ?>
  ...
  <h2>Ανανέωση υλικού - Δέματα Α</h2>
 
  <?php 
    if(isset($_POST['submit_a_material'])) {
      $stmt = $conn->stmt_init();
      $sql = 'UPDATE a_materials SET name = ?, quantity = ?, efedroi = ? WHERE id = ?';
	...
	if ($stmt->affected_rows > 0)
	  echo "<div class='alert alert-info fade in'><button class='close' data-dismiss='alert' type='button'>×</button>Ανανεώθηκε το υλικό ".$material_name." - Δέμα Α</div>";
	}
}
  ?>
